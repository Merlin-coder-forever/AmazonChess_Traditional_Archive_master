...
void CAmazonsView::ResetVariables()
{
	...
	m_strGameState = _T("");
	m_strDepth.Format(_T("深度:"));
	m_strNodes.Format(_T("节点:"));
	m_strMoves.Format(_T("得分:"));
	m_nPlayerTimer = 0;
	m_nTotalTimer = 0;
}
...
void CAmazonsView::OnDraw(CDC* pDC)
{
	...
	font.CreatePointFont(130, _T("黑体"));
	...
	for (i = 0; i < 10; i++)//写数字
	{
		...
		m_clientMemDC.TextOut(int(m_ptTopLeft2.x + (i + 0.5)*m_nCellWidth), m_ptTopLeft2.y - 26, strX);//横上
		m_clientMemDC.TextOut(int(m_ptTopLeft2.x + (i + 0.5)*m_nCellWidth), m_ptTopLeft2.y + 5 + 10 * m_nCellWidth, strX);//横下
		m_clientMemDC.TextOut(m_ptTopLeft2.x - 24, int(m_ptTopLeft2.y + (i + 0.3)*m_nCellWidth), strY);//竖左
		m_clientMemDC.TextOut(m_ptTopLeft2.x + 6 + 10 * m_nCellWidth, int(m_ptTopLeft2.y + (i + 0.3)*m_nCellWidth), strY);//竖右
	}
	//11条横线
	...
	CString drawText = _T(" ");
	drawText += g_pGameController->m_bPlayer1Black ? _T("黑方阵营·") : _T("红方阵营·");
	drawText += GetPlayerName(g_pGameController->m_nPlayer1Type);
	
	m_clientMemDC.TextOut(ptTemp.x + nMargin, ptTemp.y + 10, drawText);
	//
	drawText = _T(" ");
	drawText += g_pGameController->m_bPlayer1Black ? _T("红方阵营·") : _T("黑方阵营·");
	...
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 300, ptTemp.y + 24 , drawText + _T("先行"));
	...
	//绘制表格
	*/
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 15, ptTemp.y + 48, "共计");			//列2横2
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 15, ptTemp.y + 73, "当前");		//列3横3
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 30 + 70, ptTemp.y + 23, g_pGameController->m_bPlayer1Black ? _T("黑方") : _T("红方"));		//列2横1
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 15 + 57, ptTemp.y + 48, m_strPlayer1TotalTime);									//列2横2
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 15 + 57, ptTemp.y + 73, m_strPlayer1Timer);										//列2横3
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 30 + 185, ptTemp.y + 23, g_pGameController->m_bPlayer1Black ? _T("红方") : _T("黑方"));	//列3横1
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 15 + 172, ptTemp.y + 48, m_strPlayer2TotalTime);									//列3横2
	m_clientMemDC.TextOut(ptTemp.x + nMargin + 15 + 172, ptTemp.y + 73, m_strPlayer2Timer);										//列3横3
...
void CAmazonsView::UpdateRightInfo()
{
	...
	else if (m_nGameState == GAME_STATE_PLAYING)
	{
		...
		if (m_bComputerThinking)
		{
			...
			m_strDepth.Format(_T("深度: %d"), g_pAI->m_nOutDepth);
			m_strNodes.Format(_T("节点: %d"),g_pAI->m_nOutNumOfNodes);
			m_strMoves.Format(_T("得分: %d"), g_pAI->m_nOutMaxValue);
		}
