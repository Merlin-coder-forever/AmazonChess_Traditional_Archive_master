...
Move SearchEngine::searchMove(BYTE position[][12], bool side, int &md_, int &score_, int &node_) {
    ...
    //可以定义为开局的随机数目 所有开局均为一步评价排位靠前的 0-1为对称棋局以此类推 目前8种开局
    int selekt = rand() % 8;;
...
bool testMove(int depth, int count, Move mv) {
    ...
    for (int i = 0; i < count; i++) {
        if (mg->moveLine[depth][i].barX == mv.barX
            ...
            && mg->moveLine[depth][i].y == mv.y) { return true; }
    ...
    switch (nPhase) {
        ...
        case PHASE_CreatePM:
            while (curcount < nCount) {
                ...
                curcount++;
                if (xor(moves, killer1) && xor(moves, killer2) && xor(curhashmove, moves)) { return moves; }
...
////////////////////////////////////////////////////////
    ...
    while (xor(mv= Rs.Next(depth), nullMove)) {
        ...
        if (best == -20000) {
            score = -moveScore(-20000, 20000, depth - 1, -node_type);
        }    ////first blood  here nodetype must be 0
        else {
            score = -moveScore(-best - .0001, -best, depth - 1, (node_type == CUT_NODE ? ALL_NODE
                                                                                       : CUT_NODE));  ////fresh meat  here nodetype must be 1
            if (score > best) {
...
/*   if(node_type==CUT_NODE && depth>2 )
{
  ...
  int cc=0,mm=0;
    ...
    if (best == -20000) { return (nDistance - 20000); }
...
int SearchEngine::sortMove(int depth, int count, bool flag, double score) {
    ...
    if (flag = 0) {
        for (j = 1; j <= count - 1; j++)
            for (i = 0; i < count - j; i++)
                if (mg->moveLine[depth][i].score <
                    mg->moveLine[depth][i + 1].score) {
...
BYTE SearchEngine::GetHistoryScore(Move *MOVE) {
    int number;
    number = (MOVE->priviousX - 1) + (MOVE->priviousY - 1) * 10 + (MOVE->x - 1) * 100 + (MOVE->priviousY - 1) * 1000 +
             (MOVE->barX - 1) * 10000 + (MOVE->barY - 1) * 100000;
