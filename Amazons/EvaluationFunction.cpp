...
void EvaluationFunction::KingMoveSearch()
{
	...
	for(i=0;i<12;i++)
	{
		for(j=0;j<12;j++)
		{
			...
			{
				if(currentPosition[i-1][j-1]==0)
				{kingMoveWhite[i-1][j-1]=1;}
				
                if(currentPosition[i-1][j]==0)
				{kingMoveWhite[i-1][j]=1;}

                if(currentPosition[i-1][j+1]==0)
				{kingMoveWhite[i-1][j+1]=1;}

                if(currentPosition[i][j-1]==0)
	...
	for(int n=1;n<6;n++)
	{
		for(k=0;k<12;k++)
		{
			for(int l=0;l<12;l++)
			{
                ...
                i=k;j=l;
				while((queenMoveWhite[i-1][j-1]==0||queenMoveWhite[i-1][j-1]==n+1)&&currentPosition[i-1][j-1]==0)
				{queenMoveWhite[i-1][j-1]=n+1;i--;j--;}
                i=k;j=l;
				while((queenMoveWhite[i-1][j]==0||queenMoveWhite[i-1][j]==n+1)&&currentPosition[i-1][j]==0)
				{queenMoveWhite[i-1][j]=n+1;i--;}
                i=k;j=l;
				while((queenMoveWhite[i-1][j+1]==0||queenMoveWhite[i-1][j+1]==n+1)&&currentPosition[i-1][j+1]==0)
				{queenMoveWhite[i-1][j+1]=n+1;i--;j++;}
                i=k;j=l;
				while((queenMoveWhite[i][j-1]==0||queenMoveWhite[i][j-1]==n+1)&&currentPosition[i][j-1]==0)
				{queenMoveWhite[i][j-1]=n+1;j--;}
                i=k;j=l;
				while((queenMoveWhite[i][j+1]==0||queenMoveWhite[i][j+1]==n+1)&&currentPosition[i][j+1]==0)
				{queenMoveWhite[i][j+1]=n+1;j++;}
                i=k;j=l;
				...
				{
                i=k;j=l;
				while((queenMoveBlack[i-1][j-1]==0||queenMoveBlack[i-1][j-1]==n+1)&&currentPosition[i-1][j-1]==0)
				{queenMoveBlack[i-1][j-1]=n+1;i--;j--;}
                i=k;j=l;
				while((queenMoveBlack[i-1][j]==0||queenMoveBlack[i-1][j]==n+1)&&currentPosition[i-1][j]==0)
				{queenMoveBlack[i-1][j]=n+1;i--;}
                i=k;j=l;
				while((queenMoveBlack[i-1][j+1]==0||queenMoveBlack[i-1][j+1]==n+1)&&currentPosition[i-1][j+1]==0)
				{queenMoveBlack[i-1][j+1]=n+1;i--;j++;}
                i=k;j=l;
				while((queenMoveBlack[i][j-1]==0||queenMoveBlack[i][j-1]==n+1)&&currentPosition[i][j-1]==0)
				{queenMoveBlack[i][j-1]=n+1;j--;}
                i=k;j=l;
				while((queenMoveBlack[i][j+1]==0||queenMoveBlack[i][j+1]==n+1)&&currentPosition[i][j+1]==0)
				{queenMoveBlack[i][j+1]=n+1;j++;}
                i=k;j=l;
...
double EvaluationFunction::Evaluation(BYTE position[][12],int side)
{
   ec++;
	...
	KingMoveSearch();
	QueenMoveSearch();
	t1Count();t2Count();c1Count();c2Count();
	wCount();
    double a=(5/(w+5));
    double b=(w/(w+20));
	double c=(1-(a+b))/2;
